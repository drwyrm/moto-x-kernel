 ...
 * Copyright (c) 2015, VadimTk https://github.com/vadimtk/
 *
 ...
 *
 * A simple hotplugging driver optimized for Six-Core CPUs
 */
...
static ssize_t v4tkplug_suspend_cpus_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d", suspend_cpu_num);
}

static ssize_t v4tkplug_suspend_cpus_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int val;
	sscanf(buf, "%d", &val);
	if(val < 1 || val > 6)
...
static ssize_t v4tkplug_sampling_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d", sampling_time);
}

static ssize_t __ref v4tkplug_sampling_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int val;
	sscanf(buf, "%d", &val);
	if(val > 50)
...
static ssize_t v4tkplug_hp_enabled_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d", tplug_hp_enabled);
}

static ssize_t __ref v4tkplug_hp_enabled_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t v4tkplug_tb_enabled_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d", touch_boost_enabled);
}

static ssize_t __ref v4tkplug_tb_enabled_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int val;
	sscanf(buf, "%d", &val);
	switch(val)
...
static ssize_t v4tkplug_load_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d", load_threshold);
}

static ssize_t __ref v4tkplug_load_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int val;
	sscanf(buf, "%d", &val);
	if(val > 10)
...
{
       return sprintf(buf, "V4TKPlug %u.%u", DRIVER_VERSION, DRIVER_SUBVER);
}

static struct kobj_attribute v4tkplug_ver_attribute =
       __ATTR(version,
               0444,
               v4tkplug_ver_show, NULL);

static struct kobj_attribute v4tkplug_suspend_cpus_attribute =
       __ATTR(suspend_cpus,
               0666,
               v4tkplug_suspend_cpus_show, v4tkplug_suspend_cpus_store);

static struct kobj_attribute v4tkplug_sampling_attribute =
       __ATTR(sampling_rate,
               0666,
               v4tkplug_sampling_show, v4tkplug_sampling_store);

static struct kobj_attribute v4tkplug_load_attribute =
       __ATTR(load_threshold,
               0666,
               v4tkplug_load_show, v4tkplug_load_store);

static struct kobj_attribute v4tkplug_hp_enabled_attribute =
       __ATTR(hotplug_enabled,
               0666,
               v4tkplug_hp_enabled_show, v4tkplug_hp_enabled_store);

static struct kobj_attribute v4tkplug_tb_enabled_attribute =
       __ATTR(touch_boost,
               0666,
               v4tkplug_tb_enabled_show, v4tkplug_tb_enabled_store);

static struct attribute *v4tkplug_attrs[] =
    {
        &v4tkplug_ver_attribute.attr,
        &v4tkplug_suspend_cpus_attribute.attr,
        &v4tkplug_sampling_attribute.attr,
        &v4tkplug_load_attribute.attr,
        &v4tkplug_hp_enabled_attribute.attr,
        &v4tkplug_tb_enabled_attribute.attr,
        NULL,
    };

static struct attribute_group v4tkplug_attr_group =
    {
        .attrs = v4tkplug_attrs,
    };

static struct kobject *v4tkplug_kobj;

static int __init v4tkplug_init(void)
{
        int ret = 0;
        int sysfs_result;
        printk(KERN_DEBUG "[%s]\n",__func__);

        v4tkplug_kobj = kobject_create_and_add("thunderplug", kernel_kobj);
        ...
        sysfs_result = sysfs_create_group(v4tkplug_kobj, &v4tkplug_attr_group);
...
MODULE_AUTHOR("VadimTk https://github.com/vadimtk/");
MODULE_DESCRIPTION("Hotplug driver for SixCore CPU");
late_initcall(v4tkplug_init);
